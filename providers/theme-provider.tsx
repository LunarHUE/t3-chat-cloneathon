...
export function ThemeToggle({ className, ...props }: ThemeToggleProps) {
  ...
  const toggleDarkMode = async (theme: string) => {
    if (!ref.current || !document.startViewTransition || prefersReducedMotion) {
      setTheme(theme);
      return;
    }
  ...
  if (!mounted) {
    return (
      <Button
        ...
        disabled
      >
        <Skeleton className="size-4 bg-foreground/50" />
      </Button>
    );
  }
    ...
    >
      <AnimatePresence mode="wait" initial={false}>
        {theme === "dark" ? (
          <motion.svg
            key="sun"
            className="size-4 transition-all animate-spin-slow"
